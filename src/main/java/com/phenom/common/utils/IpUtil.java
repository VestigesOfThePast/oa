...
//        System.out.println(getLocalAddress0());
//    }
    ...
    public static HashSet<String> getLocalAddressList() {
        InetAddress localAddress = null;

        HashSet<String> inetAddressArrayList = new HashSet<>();

        try {
            Enumeration<NetworkInterface> interfaces = NetworkInterface.getNetworkInterfaces();
            if (null == interfaces) {
                inetAddressArrayList.add(localAddress.getHostAddress());
                    ...
                    while (addresses.hasMoreElements()) {
                        try {
                            ...
                            if (addressItem != null) {
                                try {
                                    if(addressItem.isReachable(100)){
                                        inetAddressArrayList.add(addressItem.getHostAddress());
                                    }
                                } catch (IOException e) {
                                    // ignore
                                }
